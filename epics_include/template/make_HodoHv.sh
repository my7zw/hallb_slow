...
for index in "${!pl[@]}"; do
    ...
    echo 'record(calc,"st_'${pl[$index]}'"){'  >> $temporary
    echo '  field(DESC,"st_'${pl[$index]}' group status")'  >> $temporary
    echo '  field(PINI,"1")'  >> $temporary
    echo '  field(INPA,"st_'${pl[$index]}'_DV_A CPP MSS")' >> $temporary
    echo '  field(INPB,"st_'${pl[$index]}'_DV_B CPP MSS")' >> $temporary    
    echo '  field(INPC,"st_'${pl[$index]}'_MV_A CPP MSS")' >> $temporary
    echo '  field(INPD,"st_'${pl[$index]}'_MV_B CPP MSS")' >> $temporary    
    ...
    echo 'record(calc,"st_'${pl[$index]}'_DV_A"){'  >> $temporary
    echo '  field(DESC,"st_'${pl[$index]}'_DV_A group status")'  >> $temporary
    echo '  field(PINI,"1")'  >> $temporary
    for (( c=0; c<${ch2[$index]}; c++ ))
    do
	chn=$(( $c + 1 ))
	echo '  field(INP'${inp[$c]}',"'${pl[$index]}'_'$chn'_DV CPP MSS")' >> $temporary
    done
    echo '  field(CALC,"A")'  >> $temporary
    echo '}'  >> $temporary
    echo " "  >> $temporary

    echo 'record(calc,"st_'${pl[$index]}'_DV_B"){'  >> $temporary
    echo '  field(DESC,"st_'${pl[$index]}'_DV_B group status")'  >> $temporary
    echo '  field(PINI,"1")'  >> $temporary
    for (( c=${ch2[$index]}; c<${ch1[$index]}; c++ ))
    do
	c2=$(( $c - ${ch2[$index]} ))
	chn=$(( $c + 1 ))
	echo '  field(INP'${inp[$c2]}',"'${pl[$index]}'_'$chn'_DV CPP MSS")' >> $temporary
    done
    echo '  field(CALC,"A")'  >> $temporary
    echo '}'  >> $temporary
    echo " "  >> $temporary
